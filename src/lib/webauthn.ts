...
/**
 * Enrolls a platform authenticator and wraps `pin` with a PRF-derived key.
 * Returns `null` if the browser/authenticator doesn't support the PRF extension.
 */
export async function enrollBiometric(pin: string): Promise<WebAuthnEnrollment | null> {
	...
	const prfEnabled = (credential.getClientExtensionResults() as { prf?: { enabled?: boolean } }).prf
		?.enabled;
	if (!prfEnabled) return null; // Authenticator doesn't support PRF — biometric unavailable.
	...
	return { credentialId: toBase64Url(credential.rawId), prfSalt, wrappedPin };
}
